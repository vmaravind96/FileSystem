  ...
  override def apply(state: State): State = {
    ...
    val absolutePath = {
      if (dir.startsWith(Directory.SEPARATOR)) {
        ...
        workingDir.path + Directory.SEPARATOR + dir
      }
    }
  ...
  def findDestinationEntry(root: Directory, path: String): DirEntry = {
    ...
    def findEntryHelper(curDirectory: Directory, tokens: List[String]): DirEntry = {
      if (tokens.isEmpty || tokens.head.isEmpty) {
        ...
        curDirectory.findEntry(tokens.head)
      } else {
        val nextDir = curDirectory.findEntry(tokens.head)
        if (nextDir == null || !nextDir.isDirectory) {
          null
        } else {
          findEntryHelper(nextDir.asDirectory, tokens.tail)
        }
      }
    }
    ...
    def collapseTokens(tokens: List[String], accumulator: List[String]): List[String] = {
      if (tokens.isEmpty) {
        ...
        collapseTokens(tokens.tail, accumulator)
      } else if (tokens.head.equals("..")) {
        if (accumulator.isEmpty) {
          null
        } else {
          collapseTokens(tokens.tail, accumulator.init)
        }
      }
      else {
        collapseTokens(tokens.tail, accumulator :+ tokens.head)
      }
    }
    ...
    */
    ...
    if (cleanedTokens == null) {
      null
    } else {
      findEntryHelper(root, cleanedTokens)
    }
  }
}
